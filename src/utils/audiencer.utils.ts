...
export const fixAudienceName = <T extends { name: string }>({
  name,
  ...e
}: T) => ({
  ...e,
...
export const filterByDateTime =
  <
    T extends { items: { startAt: string | Date; endAt: string | Date }[] },
  >(filterDateTime: {
    date1: Dayjs | null;
    date2: Dayjs | null;
    time1: Dayjs | null;
    ...
    if (date1 || date2 || time1 || time2) {
      items = items.filter((l) => {
        ...
        const startTime = dayjs(
          startAt.get('hour') + ':' + startAt.get('m'),
          'HH:mm',
        );
        const endTime = dayjs(
          endAt.get('hour') + ':' + endAt.get('m'),
          'HH:mm',
        );
...
export const filterByLessonType = <
  T extends { items: { type: LessonFlags }[] },
>(
  ...
  return (e: T) => ({
    ...
    items:
      lessonFlagFilter === 0
        ? e.items
        : e.items.filter(
            (item) => (lessonFlagFilter & item.type) === item.type,
          ),
  });
};
