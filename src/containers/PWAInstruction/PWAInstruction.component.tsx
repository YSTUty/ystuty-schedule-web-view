...
const PWAInstructionComponent = () => {
  const [deferredPrompt, setAndroidDeferredPrompt] =
    ...
    <div className={styles.Container}>
      <div className={styles.Caption}>
        <div className={styles.CaptionText}>
          ...
          {formatMessage(
            { id: 'pwa.h1' },
            { device: userOS, version: appVersion.version },
          )}
        </div>
        <button
          type="button"
      ...
      {infoByOS.map((info) => (
        <div className={styles.Item} key={info.id}>
          <div className={styles.ItemHead}>
            ...
            {info.captionIcon &&
              React.createElement(info.captionIcon, {
          ...
          <div className={styles.ItemBottom}>
            {info.id === 1
              ? formatMessage(
                  { id: info.text },
                  { browser: userOS === 'android' ? 'Chrome' : 'Safari' },
                )
      ...
      {userOS === 'android' && (
        <div className={styles.Alert}>
          {formatMessage({ id: 'pwa.android.alert' })}
        </div>
      )}
      {userOS === 'ios' && (
        <PWAPrompt
