...
import * as dxSchedulerTypes from 'devextreme/ui/scheduler';

import {
    Scheduler as SchedulerOLD,
    WeekView,
    Appointments,
    AppointmentTooltip,
    AppointmentForm,
...
import {
    ...
    StyledAppointmentsAppointmentContent_NEW,
    StyledGrid,
    StyledIcon,
...
const Appointment_OLD = ({ data, ...restProps }: Appointments.AppointmentProps) => (
    <StyledAppointmentsAppointment
        {...restProps}
...
const AppointmentContent = (model: dxSchedulerTypes.AppointmentTemplateData) => {
    const data = model.targetedAppointmentData! as dxSchedulerTypes.AppointmentTemplateData['targetedAppointmentData'] &
        LessonData & { group?: string };
    ...
    return (
        <StyledAppointmentsAppointmentContent_NEW /* {...restProps}  data={data}*/ title={title}>
...
const AppointmentTooltipContent = (model: dxSchedulerTypes.AppointmentTooltipTemplateData) => {
    const appointmentData =
        model.appointmentData! as dxSchedulerTypes.AppointmentTooltipTemplateData['appointmentData'] &
            LessonData & { group?: string };
    console.log('AppointmentTooltip_NEW', appointmentData);
    return (
        <div>
...
const NewMaterialContainer = () => {
    ...
    return (
        <Paper style={{ height: 'calc(100vh - 56px)' }}>
            <Scheduler
                ...
                height={'100vh'}
                //
                appointmentRender={AppointmentContent}
                appointmentTooltipRender={AppointmentTooltipContent}
            >
                <Resource
                    dataSource={getResources(selectedGroups)[0].instances}
                    allowMultiple={true}
                    fieldExpr="typeArr"
                    label="Type"
                    useColorAsDefault={true || mainResourceName === 'typeArr'}
                />
                <Resource
                    dataSource={getResources(selectedGroups)[1].instances}
                    fieldExpr="group"
                    label="Группа"
                    useColorAsDefault={mainResourceName === 'group'}
                />
            </Scheduler>
        </Paper>
    );
    ...
    return (
        <Paper style={{ height: 'calc(100vh - 56px)' }}>
            <SchedulerOLD locale="ru" data={dataMemo} firstDayOfWeek={1}>
                <ViewState />
                {hasGroupingGroups && (
                    <GroupingState
                ...
                {/* <Appointments
                    appointmentComponent={Appointment_OLD}
...
export default NewMaterialContainer;
